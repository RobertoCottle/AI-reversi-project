...
#meant for opening moves
OPENING_BOOK = {}

def load_opening_book(path: str = "opening_book.json"):
    global OPENING_BOOK
    if os.path.exists(path):
        with open(path, "r") as f:
            OPENING_BOOK = json.load(f)
        print(f"[BOOK] Loaded {len(OPENING_BOOK)} positions")
    else:
        print("[BOOK] No opening book found, running without it")

def book_move(board: np.ndarray, player: int):
    # Cast to float64 to match reversi class np.zeros([8,8]) default dtype
    key = board.astype(np.float64).tobytes().hex() + str(player)
    if key in OPENING_BOOK:
        move = tuple(OPENING_BOOK[key])
        print(f"[BOOK] Hit — playing {move}")
        return move
    return None

#------ End of Opening moves section ------------------------------------------#
...
def choose_move(board: np.ndarray, player: int):
    # Check opening book first — free move, zero search
    move = book_move(board, player)
    if move:
        return move
    TT.clear()
